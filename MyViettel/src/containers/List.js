...
export default class List extends Component {
    ...
    async getDataProduct() {
        ...
        let rs = await dataService.getListPromotions(
            this.state.skip, this.state.limit, this.props.category, 'gift,giftAnother,exchange', this.state.latitude ? "location" : 'new', null, this.state.latitude, this.state.longitude, null, null, 'exchange');
        this.setState({
            dataProduct: rs.data,
            loadding: false,
            refreshing: false,
    ...
    async loadMoreProduct() {
        if (this.state.stopLoad == false && !this.isLoading) {
            ...
            let arrNew = await dataService.getListPromotions(this.state.dataProduct.length, this.state.limit, this.props.category, 'gift,giftAnother,exchange', this.state.latitude ? "location" : 'new', null, this.state.latitude, this.state.longitude, null, null, 'exchange');
